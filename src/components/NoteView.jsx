...
function NoteView() {
    ...
    const back = async(conditional) => {
        try { 
            const response = await httpClient.get(`https://iarchiveapp-env.eba-ezit6mbr.us-east-1.elasticbeanstalk.com/users/${note.creator}`)
    ...
    const confirmEdit = async() => {
        try {
            const response = await httpClient.put(`https://iarchiveapp-env.eba-ezit6mbr.us-east-1.elasticbeanstalk.com/note/${note._id['$oid']}`, editPrefs)
                if (response.status === 200) {
                    console.log(response.data.message)
    ...
    const deleteNote = async () => {
        try {
            const response = await httpClient.delete(`https://iarchiveapp-env.eba-ezit6mbr.us-east-1.elasticbeanstalk.com/note/${note._id['$oid']}`)
            if (response.status === 200) {
              console.log(response.data.message)
              back()
            } 
